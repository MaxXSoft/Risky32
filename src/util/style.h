#ifndef RISKY32_UTIL_STYLE_H_
#define RISKY32_UTIL_STYLE_H_
...
inline StreamWrapper operator<<(std::ostream &os, StyleFormat &&sf) {
  StreamWrapper sw(os);
  sw << std::move(sf);
  return sw;
}

inline StreamWrapper operator<<(std::ostream &os, Style s) {
  StreamWrapper sw(os);
  sw << s;
  return sw;
}

#endif  // RISKY32_UTIL_STYLE_H_
